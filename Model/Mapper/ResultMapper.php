...
class ResultMapper extends AbstractDataMapper implements ResultMapperInterface {
	...
	public function insert(ShotInterface $shot, UserInterface $user, $hit, $distance_target_impact){
		return $this->adapter->insert(
			...
			array(
				...
				'id_user' => $user->getId(),
				'hit' => $hit,
				'distance_target_impact' => $distance_target_impact
			)
		);
	}
	...
	public function getTopAcurateUsersByLimit($number){
		$sql = "SELECT id_user, sum(hit) as 'hits', AVG(distance_target_impact) as 'avg-closed-target'
								FROM result
								GROUP BY  id_user
								ORDER BY hits DESC
								LIMIT " . $number;
	...
	protected function createEntity(array $row){
		...
		$result = new Result($shot, $user, $row['hit'], $row['distance_target_impact']);
		$result->setId($row['id']);
		return $result;
	}
}
